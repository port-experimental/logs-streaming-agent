...
exports.handleBuildAction = handleBuildAction;
const shared_1 = require("@cicd/shared");
async function handleBuildAction(message, consumer) {
    ...
    const ciProvider = shared_1.pluginRegistry.getProvider(provider);
    if (!ciProvider) {
        throw new Error(`CI/CD provider '${provider}' is not registered. Available providers: ${shared_1.pluginRegistry.getProviderNames().join(', ')}`);
    }
    ...
    try {
        ...
        await ciProvider.streamLogs(buildInfo.buildId, async (logChunk) => {
            logBuffer += logChunk;
        ...
        if (isSuccess) {
            await consumer.addActionRunLog(runId, `✅ Successfully completed build #${buildInfo.buildNumber}!\nDuration: ${duration}s`);
        }
        else {
        ...
        await consumer.addActionRunLog(runId, `❌ Build failed: ${errorMsg}`);
        throw error;
    }
}
